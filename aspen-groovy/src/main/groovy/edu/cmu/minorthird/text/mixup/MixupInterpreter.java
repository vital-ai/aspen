...
public class MixupInterpreter{
	...
	private void evaluate(Statement statement){
		...
		if("defDict".equals(keyword)){
			...
			}else if(statementType==Statement.TRIE || statementType==Statement.ITRIE){
				boolean caseSensitive = statementType==Statement.TRIE;
				labels.defineTrie(phraseList, caseSensitive);
				while(input.hasNext()){
